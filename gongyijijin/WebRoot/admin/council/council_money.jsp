...
<script type="text/javascript">
	...
	function deleteUser(id) {
		if (confirm("确认驳回吗？")) {
			location.href="secondMoney_update.action?consent="+id;
		}
	...
	function motifyUser(id){
		if (confirm("确认同意吗？")) {
			location.href="secondMoney_update2.action?reject="+id;
		}
	}
	...
	window.onload = function(){
		...
		$("#selectBtn").click(function(){
			...
			$.ajax({
				...
				alert("11111111"),
					alert(data.list2[0].moneyId);
					$("#tbody").empty();
					var str="";
					for(var i=0;i<data.list2.length;i++){
					
						str += "<tr>"
						str+="<td>"+data.list2[i].moneyId+"</td>";
						str+="<td>"+data.list2[i].proName+"</td>";
						str+="<td>"+data.list2[i].firstCheckEmp+"</td>";
						str+="<td>"+data.list2[i].applyReason+"</td>";
						str+="<td>"+data.list2[i].money+"</td>";
						
					
						str+="<td><button class='btn btn-danger btn-xs' onclick='motifyUser("+data.list2[i].moneyId+")' name='consent'  value='${"+data.list2[i].moneyId+"}'>同意</button><button class='btn btn-danger btn-xs' onclick='deleteUser("+data.list2[i].moneyId+")'  name='reject' value='${"+data.list2[i].moneyId+"}'>驳回</button></td>";
						str+="</tr>";
					}
					$("#tbody").html(str);
					...
					<div class="col-xs-5">
						...
						<div class="input-group">
							<span class="input-group-addon" id="basic-addon1">用户名:</span> <input
								type="text" class="form-control" placeholder="外现身"
								aria-describedby="basic-addon1" id="council_ProName" name="proName"/>
						</div>
					</div>
					<div class="col-xs-5">
						<div id="council_div">按项目申请资金额度查询</div>
						<div class="input-group">
							<span class="input-group-addon" id="basic-addon1">资金额度:</span> <input
								type="text" class="form-control" placeholder="默认为范围为上下一千"
					...
					<div class="col-xs-5">
						...
						<div class="input-group">
							<span class="input-group-addon" id="basic-addon1">负责人:</span> <input
								type="text" class="form-control" placeholder="张三"
			...
			<table class="table">
				...
				<c:forEach items="${bean.list}" var="aa" varStatus="ab">
					<tr>
						...
						<td> ${aa.moneyId}</td>
						<td>${aa.proName}</td>
						<td>${aa.firstCheckEmp}</td>
						<td>${aa.applyReason}</td>
						<td>${aa.money}</td>
						<td>
						
							<a href="pro_update.action"><button class="btn btn-danger btn-xs"
								onclick="deleteUser(${aa.moneyId})">同意</button></a>
							<a href="pro_update1.action"><button class="btn btn-danger btn-xs"
								onclick="motifyUser(${aa.moneyId})">驳回</button></a>
						</td>
